...
DASHI_NHANES = DASHI_NHANES_FPED(NHANES_20172018$FPED_IND, NHANES_20172018$NUTRIENT_IND, NHANES_20172018$FPED_IND2, NHANES_20172018$NUTRIENT_IND2)
...
y_case1 = c(DASHI_DASH$DASHI_ALL[2]/50, DASHI_DASH$DASHI_ALL[3]/50, DASHI_DASH$DASHI_ALL[5]/50, MEDI_PREDIMED$MEDI_ALL[1]/11,  MEDI_PREDIMED$MEDI_ALL[2]/11, MEDI_PREDIMED$MEDI_ALL[3]/11, mean(DASHI_NHANES$DASHI_ALL, na.rm=TRUE)/50, mean(MEDI_NHANES$MEDI_ALL, na.rm=TRUE)/11)
...
y_case3 = c(mean(DASHI_NHANES$DASHI_ALL, na.rm=TRUE)/50, mean(MEDI_NHANES$MEDI_ALL, na.rm=TRUE)/11, mean(DASH_NHANES$DASH_ALL, na.rm=TRUE)/40, mean(MED_NHANES$MED_ALL, na.rm=TRUE)/9, mean(HEI2020_NHANES_1718$HEI2020_ALL, na.rm=TRUE)/100, mean(AHEI_NHANES$AHEI_ALL, na.rm=TRUE)/110, 0.75)
# multiply all y values by 100 to get the percentage
y_case3 = y_case3*100
...
ggplot(df_case3, aes(x_case3, y_case3, fill=x_case3)) + 
  geom_bar(stat = "identity") + theme_bw() + 
  labs(x = "Dietary Index", y = "Mean dietary index percentile", title = "Dietary Indexes in NHANES 2017-2018", fill = "Group") + 
  theme(plot.title = element_text(size=22)) +
  theme(axis.title.x = element_text(size=18)) +
  theme(axis.title.y = element_text(size=18)) +
  theme(axis.text.x = element_text(size=14)) +
  theme(aspect.ratio = 0.5) +
  # add numeric labels to the bars
  geom_text(aes(label = round(y_case3, 2)), vjust = -0.5, size = 3.5) +
  # rotate the x axis labels by 90 degrees
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
