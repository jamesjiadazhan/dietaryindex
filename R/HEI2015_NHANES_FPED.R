  ...
  if (!is.null(FPED_PATH) & !is.null(NUTRIENT_PATH) & !is.null(FPED_PATH2) & !is.null(NUTRIENT_PATH2)) {
    COHORT12 <- inner_join(COHORT, COHORT2, by = "SEQN") %>%
      mutate(
        ...
        TOTALKCAL = (TOTALKCAL.x + TOTALKCAL.y)/2,
        TOTALFRT_SERV = (TOTALFRT_SERV.x + TOTALFRT_SERV.y)/2,
        FRT_SERV = (FRT_SERV.x + FRT_SERV.y)/2,
        VEG_SERV = (VEG_SERV.x + VEG_SERV.y)/2,
        GREENNBEAN_SERV = (GREENNBEAN_SERV.x + GREENNBEAN_SERV.y)/2,
        TOTALPRO_SERV = (TOTALPRO_SERV.x + TOTALPRO_SERV.y)/2,
        SEAPLANTPRO_SERV = (SEAPLANTPRO_SERV.x + SEAPLANTPRO_SERV.y)/2,
        WHOLEGRAIN_SERV = (WHOLEGRAIN_SERV.x + WHOLEGRAIN_SERV.y)/2,
        DAIRY_SERV = (DAIRY_SERV.x + DAIRY_SERV.y)/2,
        FATTYACID_SERV = (FATTYACID_SERV.x + FATTYACID_SERV.y)/2,
        REFINEDGRAIN_SERV = (REFINEDGRAIN_SERV.x + REFINEDGRAIN_SERV.y)/2,
        SODIUM_SERV = (SODIUM_SERV.x + SODIUM_SERV.y)/2,
        ADDEDSUGAR_SERV = (ADDEDSUGAR_SERV.x + ADDEDSUGAR_SERV.y)/2,
        SATFAT_SERV = (SATFAT_SERV.x + SATFAT_SERV.y)/2
      ) %>%
      dplyr::select(SEQN, HEI2015_ALL, HEI2015_TOTALFRT, HEI2015_FRT, HEI2015_VEG, HEI2015_GREENNBEAN,
                    HEI2015_TOTALPRO, HEI2015_SEAPLANTPRO, HEI2015_WHOLEGRAIN, HEI2015_DAIRY,
                    HEI2015_FATTYACID, HEI2015_REFINEDGRAIN, HEI2015_SODIUM, HEI2015_ADDEDSUGAR,
