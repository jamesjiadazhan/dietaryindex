...
DASHI_NHANES_FPED = function(FPED_IND_PATH=NULL, NUTRIENT_IND_PATH=NULL, FPED_IND_PATH2=NULL, NUTRIENT_IND_PATH2=NULL){
  ...
  if (!is.null(FPED_IND_PATH) & !is.null(NUTRIENT_IND_PATH)) {
    ...
    if (!is.null(FPED_IND_PATH2) & !is.null(NUTRIENT_IND_PATH2)) {
        ...
        COHORT2 = COHORT2 %>%
            dplyr::mutate(
            LOWF_MILK_SERV = case_when(
                DR2IFDCD.x %in% LOWF_MILK ~ DR2I_D_MILK,
                TRUE ~ 0
            ),
            LOWF_CHEESECREAM_SERV = case_when(
                DR2IFDCD.x %in% LOWF_CHEESE ~ DR2I_D_CHEESE*4/1.5,
                TRUE ~ 0
            ),

            SNACKS_SWEETS_SERV_IND = case_when(
                DR2IFDCD.x %in% SNACKS_SWEETS ~ DR2IGRMS.x/23,
                TRUE ~ 0
            ),

            FAT_OIL_SERV = case_when(
                DR2IFDCD.x %in% FAT_OIL ~ DR2IGRMS.x/14,
                TRUE ~ 0
                )
            ) %>%
            dplyr::group_by(SEQN) %>%
            dplyr::summarize(
                DASHI_TOTALKCAL = sum(DR2IKCAL),
                VEG_SERV_DASHI = sum(DR2I_V_DRKGR + (DR2I_V_REDOR_TOTAL +  DR2I_V_OTHER + DR2I_V_STARCHY_OTHER)/0.5)/(DASHI_TOTALKCAL/2000),
                FRT_FRTJ_SERV_DASHI = sum(DR2I_F_TOTAL)/(DASHI_TOTALKCAL/2000),
                NUTSLEG_SERV_DASHI = sum((DR2I_V_LEGUMES/0.5) + (DR2I_PF_NUTSDS/2.67) + (DR2I_PF_SOY/0.5))/(DASHI_TOTALKCAL/2000),
                LOWF_DAIRY_SERV_DASHI = sum(LOWF_MILK_SERV+LOWF_CHEESECREAM_SERV+DR2I_D_YOGURT)/(DASHI_TOTALKCAL/2000),
                WGRAIN_SERV_DASHI = sum(DR2I_G_WHOLE)/(DASHI_TOTALKCAL/2000),
                WHITE_MEAT_SERV_DASHI = sum((DR2I_PF_POULT + DR2I_PF_SEAFD_HI + DR2I_PF_SEAFD_LOW)/2.5)/(DASHI_TOTALKCAL/2000),
                REDPROC_MEAT_SERV_DASHI = sum((DR2I_PF_CUREDMEAT+DR2I_PF_MEAT+DR2I_PF_ORGAN)/2.5)/(DASHI_TOTALKCAL/2000),
                FATOIL_SERV_DASHI = sum(FAT_OIL_SERV)/(DASHI_TOTALKCAL/2000),
                SNACKS_SWEETS_SERV_DASHI = sum(SNACKS_SWEETS_SERV_IND)/(DASHI_TOTALKCAL/2000),
                SODIUM_SERV_DASHI = sum(DR2ISODI)/(DASHI_TOTALKCAL/2000)
            )
    ...
    if (!is.null(FPED_IND_PATH) & !is.null(NUTRIENT_IND_PATH) & !is.null(FPED_IND_PATH2) & !is.null(NUTRIENT_IND_PATH2)){
        COHORT12 <- inner_join(COHORT, COHORT2, by = "SEQN") %>%
            dplyr::mutate(
                ...
                DASHI_TOTALKCAL = (DASHI_TOTALKCAL.x + DASHI_TOTALKCAL.y)/2,
                DASHI_VEG = (DASHI_VEG.x + DASHI_VEG.y)/2,
                DASHI_FRT = (DASHI_FRT.x + DASHI_FRT.y)/2,
                DASHI_NUTSLEG = (DASHI_NUTSLEG.x + DASHI_NUTSLEG.y)/2,
                DASHI_LOWFATDAIRY = (DASHI_LOWFATDAIRY.x + DASHI_LOWFATDAIRY.y)/2,
                DASHI_WGRAIN = (DASHI_WGRAIN.x + DASHI_WGRAIN.y)/2,
                DASHI_WHITEMEAT = (DASHI_WHITEMEAT.x + DASHI_WHITEMEAT.y)/2,
                DASHI_REDPROC_MEAT = (DASHI_REDPROC_MEAT.x + DASHI_REDPROC_MEAT.y)/2,
                DASHI_FATOIL = (DASHI_FATOIL.x + DASHI_FATOIL.y)/2,
                DASHI_SNACKS_SWEETS = (DASHI_SNACKS_SWEETS.x + DASHI_SNACKS_SWEETS.y)/2,
